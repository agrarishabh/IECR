import React, { useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'
import { assets } from '../assets/assets'
import { Eye, Star, MenuIcon, XIcon, Users } from 'lucide-react'
import { useClerk, UserButton, useUser } from '@clerk/clerk-react'
import NotificationBell from './NotificationBell'

const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const { user } = useUser();
  const { openSignIn } = useClerk();
  const navigate = useNavigate();

  const closeMenu = () => setIsOpen(false);

  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      <Link to='/' className='max-md:flex-1'>
        <img src={assets.logo} alt="IECR Logo" className='w-22 h-10 logo-glow' />
      </Link>

      <div className={`max-md:absolute max-md:top-0 max-md:left-0 
        max-md:font-medium max-md:text-lg z-50 flex flex-col md:flex-row items-center 
        max-md:justify-center gap-8 min-md:px-8 py-3 max-md:h-screen min-md:rounded-full 
        backdrop-blur bg-black/70 md:bg-white/10 md:border border-gray-300/20 
        overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-72' : 'max-md:w-0'}`}>

        <XIcon
          className='md:hidden absolute right-6 top-6 w-6 h-6 cursor-pointer'
          onClick={closeMenu}
        />

        <Link
          to='/'
          className='nav-link-hover'
          onClick={() => { scrollTo(0, 0); closeMenu(); }}
        >
          Home
        </Link>

        <Link
          to='/movies'
          className='nav-link-hover'
          onClick={() => { scrollTo(0, 0); closeMenu(); }}
        >
          Indian Movies
        </Link>

        <Link
          to='/webseries'
          className='nav-link-hover'
          onClick={() => { scrollTo(0, 0); closeMenu(); }}
        >
          Indian Webseries
        </Link>
      </div>

      <div className='flex items-center gap-8'>
        {!user ? (
          <button
            onClick={openSignIn}
            className='px-4 py-1 sm:px-7 sm:py-2 bg-[#37C6CB] hover:bg-[#1FA9AF] transition rounded-full font-medium cursor-pointer'
          >
            Login
          </button>
        ) : (
          <div className="flex items-center gap-4">
            <NotificationBell />
            <UserButton>
              <UserButton.MenuItems>
                <UserButton.Action
                  label="Social Hub"
                  labelIcon={<Users width={15} />}
                  onClick={() => navigate('/social')}
                />
                <UserButton.Action
                  label="My Watchlist"
                  labelIcon={<Eye width={15} />}
                  onClick={() => navigate('/my-watchlist')}
                />
                <UserButton.Action
                  label="My Ratings"
                  labelIcon={<Star width={15} />}
                  onClick={() => navigate('/my-ratings')}
                />
              </UserButton.MenuItems>
            </UserButton>
          </div>
        )}
      </div>

      <MenuIcon
        className='max-md:ml-4 md:hidden w-8 h-8 cursor-pointer'
        onClick={() => setIsOpen(!isOpen)}
      />
    </div>
  );
};

export default Navbar;
