...
const WebseriesCard = ({ webseries }) => {
    ...
    return (
        <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:translate-y-1 transition duration-300 w-66'>
            <img src={webseries.backdrop_path} alt="" className='rounded-lg h-90 w-full object-cover object-right-bottom cursor-pointer'/>
            <p className='font-semibold mt-2 truncate'>{webseries.title}</p>
            <p className='text-sm text-gray-400 mt-2'>
                {webseries.release_year} • {webseries.seasons} • {webseries.rating} • {webseries.votes}    
            </p>
