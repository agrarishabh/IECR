...
const Movies = () => {
  ...
  // TanStack Query — cached, no loading flicker on back-navigation
  ...
  }, [movies, debouncedSearch, filters, sortKey, sortOrder]);
    ...
    <div className="relative my-40 mb-60 px-6 md:px-16 lg:px-40 xl:px-44 overflow-hidden min-h-[80vh]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        <h1 className="text-lg font-medium animate-slide-in-left heading-hover-cyan">
          Showing All Movies
          {filteredAndSorted.length !== movies.length && (
            <span className="text-sm text-gray-400 ml-2">({filteredAndSorted.length} of {movies.length})</span>
          )}
        </h1>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            ...
            <option value="runtime:desc">Runtime ↓</option>
            <option value="runtime:asc">Runtime ↑</option>
          </select>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-10">
          <div className="flex flex-wrap max-sm:justify-center gap-8">
            {paginatedMovies.map((movie, index) => (
              <MovieCard key={movie._id || movie.id} movie={movie} style={{ animationDelay: `${(index % itemsPerPage) * 0.05}s` }} />
            ))}
          </div>
        ...
        <div className="flex flex-col items-center justify-center h-40 gap-3">
          <p className="text-gray-400 text-lg">No movies match your search or filters.</p>
